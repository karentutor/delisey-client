...
// ✅ Backend API client (Mongo/Express)
const ordersApi = axios.create({
  baseURL: process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:4000',
  headers: { 'Content-Type': 'application/json' },
  withCredentials: true, // supports cookie auth for logged-in users
});
...
// ✅ What your Next.js /api/checkout should return (recommended)
...
export default function OrderPage() {
  ...
  // show pink when submit attempted
  ...
  // ✅ Require login before showing the order page
  useEffect(() => {
    let mounted = true;
    ...
    (async () => {
      try {
        ...
        // Not logged in → send to login, then come back to /order
        if (httpStatus === 401 || httpStatus === 403) {
          router.replace('/login?next=/order');
          return;
        }

        // Backend down / CORS / etc.
        setStatus({
          state: 'error',
          message:
            'Could not reach the backend on http://localhost:4000. Start the server and try again.',
        });
      }
    })();
  ...
  async function startCheckout(values: FormValues) {
    ...
    // 1) Create order in Mongo first (required)
    let orderId: string | null = null;

    try {
      const createRes = await ordersApi.post<CreateOrderResponse>('/api/orders', {
    ...
    // 2) Try Next.js Square Checkout route (axios)
    //    If it returns a URL, patch order + redirect.
    try {
      const squareRes = await axios.post<SquareCheckoutResponse>(
        '/api/checkout',
        {
          ...
          // Optional: pass your Mongo orderId so server can attach metadata / notes
      ...
      if (url) {
        // ✅ Patch order with checkout + Square metadata
        // Best effort: don't block redirect if patch fails
        try {
          await ordersApi.patch(`/api/orders/${orderId}/checkout`, {
    ...
    // Patch order to reflect fallback used (best effort)
    try {
      await ordersApi.patch(`/api/orders/${orderId}/checkout`, {
